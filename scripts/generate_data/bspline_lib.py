...
from pb_ompl.pb_ompl import fit_bspline_to_path, create_bspline_knots 
...
from bspline_merge_test import load_spline, get_new_control_point, merge_splines
from mpd.models.diffusion_models.comp_diffuser.traj_blender import blend_2_np_trajs_23d
from scipy.signal import convolve
from mpd.datasets.trajectories_dataset_waypoints import subsample_waypoints

import pdb
def to_scipy_bspline(spl, cps) : # ParametricTrajectoryBspline
    k = spl.bspline.d
    tt = spl.bspline.u 
    cc_np = to_numpy(cps)
    bspl = interpolate.BSpline(tt, cc_np, k)

    return bspl

def simple_merge_overlap(
    l_cps, # 
    r_cps, # 
    left_traj, # ParametricTrajectoryBspline
    right_traj,
    l_pts,
    r_pts, # 
    merged_spl,
    ...
    # blend trajectory 
    l_ovlp = to_numpy(l_pts.clone())
    r_ovlp = to_numpy(r_pts.clone())

    # split fixed cps
    l_spl = to_scipy_bspline(left_traj,l_cps)
    r_spl = to_scipy_bspline(right_traj,r_cps)
    l_bnd = 1 - overlap/(len(l_cps) - 5)
    r_bnd = overlap/(len(r_cps)-5)
    l_removed = split_curve_scipy(l_spl,l_bnd,1)
    l_fixed = split_curve_scipy(l_spl,0,l_bnd)
    r_removed = split_curve_scipy(r_spl,0,r_bnd)
    r_fixed = split_curve_scipy(r_spl,r_bnd,1)
    # renormalize split knots
    assert len(l_fixed.c) == len(l_cps) - overlap and  len(r_fixed.c) == len(r_cps) - overlap 
    new_knot_l = create_bspline_knots(len(l_fixed.c), deg)
    new_knot_r = create_bspline_knots(len(r_fixed.c), deg)
    l_fixed = interpolate.BSpline(new_knot_l, l_fixed.c,deg)
    r_fixed = interpolate.BSpline(new_knot_r,r_fixed.c,deg)
    # convert number of overlaps in timestep 
    #
    h_l = 1/(left_traj.bspline.num_T_pts - 1)  
    h_r = 1/(right_traj.bspline.num_T_pts - 1)
    t_idx_l = int(np.ceil(l_bnd / h_l))
    t_idx_r = int(np.floor(r_bnd / h_r))
    print(t_idx_l, t_idx_r)
    ovlp_t = min(left_traj.num_T_pts - t_idx_l, t_idx_r + 1)
    ovlp_traj = blend_2_np_trajs_23d(l_ovlp[-ovlp_t:], r_ovlp[:ovlp_t], blend_type="linear")
    # fit overlap into bspline

    params = fit_bspline_to_path(
        ovlp_traj,
        bspline_degree=deg,
        bspline_num_control_points = len(l_fixed.c), # use same
        bspline_zero_acc_at_start_and_goal=False,
        bspline_zero_vel_at_start_and_goal=False,
        debug=False,
    )
    t, cc, k = params
    ovlp_spl = interpolate.BSpline(t, cc.T, k)
    #ovlp_cps = to_torch(cc.T, **tensor_args)
    ovlp_cps = ovlp_spl.c

    
    # l_len, dim = left_traj.shape
    # r_len = right_traj.shape[0]
    # blend_traj = np.zeros((l_len + r_len - overlap, dim))
    # blend_traj[:l_len - overlap] =  l_ovlp[:-overlap]
    # blend_traj[l_len - overlap: l_len] = ovlp_traj
    # blend_traj[l_len:] = r_ovlp[overlap:]
    # print(blend_traj.shape)

    fig_cps, axs_cps = create_fig_and_axes(2, figsize=(6, 6))
    l_cps_fixed = l_fixed.c
    r_cps_fixed=  r_fixed.c
    print(l_cps_fixed[-1], l_removed.c[0], ovlp_cps[0])
    print(r_cps_fixed[0], r_removed.c[-1], ovlp_cps[-1])
    print(len(l_cps_fixed), len(ovlp_cps), len(r_cps_fixed))
    axs_cps.scatter(l_cps_fixed[:,0], l_cps_fixed[:,1], c="red", marker="o", s=10**2, zorder=100)
    axs_cps.scatter(l_removed.c[:,0], l_removed.c[:,1], c="red", marker="o", s=10**2, zorder=102, alpha=0.3)
    axs_cps.scatter(ovlp_cps[:,0], ovlp_cps[:,1], c="blue", marker="o", s=10**2, zorder=101, alpha=0.3)
    axs_cps.scatter(r_cps_fixed[:,0], r_cps_fixed[:,1], c="green", marker="o", s=10**2, zorder=100)
    axs_cps.scatter(r_removed.c[:,0], r_removed.c[:,1], c="green", marker="o", s=10**2, zorder=102, alpha=0.3)
    # axs_cps.plot(blend_traj[:, 0], blend_traj[:, 1], color="orange", linestyle="solid", linewidth=3, marker="x")
    #axs_cps.plot(l_ovlp[:, 0], l_ovlp[:, 1], color="red", linestyle="solid", linewidth=3, marker="x", alpha=0.3)
    #axs_cps.plot(r_ovlp[:, 0], r_ovlp[:, 1], color="green", linestyle="solid", linewidth=3, marker="x", alpha=0.3)
    #axs_cps.plot(blend_traj_ref, blend_traj_ref, color="gray", linestyle="solid", linewidth=3, marker="x", alpha=0.3) # ground truth 
    
    fig_cps.savefig(os.path.join("figures/robot_trajectories_blend_2.png"), bbox_inches="tight")
    ...
    # cc = to_torch(cc.T, **tensor_args)
    # print(cc.shape)
    without_merge = False
    if without_merge : 
        l_cps_fixed = to_torch(l_cps_fixed, **tensor_args)
        ovlp_cps = to_torch(ovlp_cps, **tensor_args)
        r_cps_fixed = to_torch(r_cps_fixed, **tensor_args)
        merged_cps = torch.cat([l_cps_fixed, ovlp_cps, r_cps_fixed]) 
        n_merged = merged_cps.shape[0]
        print(f"{n_merged=}")

        spans_left = len(l_cps_fixed) - deg
        ...
        return merged_cps, merged_traj 
    
    else : 
        # modify each end 
        #print(l_cps_fixed[-1], l_removed.c[0], ovlp_cps[0])
        #print(ovlp_cps)
        ovlp_cps[0] = l_cps_fixed[-1]
        ovlp_cps[-1] = r_cps_fixed[0]
        #print(ovlp_cps)
        #pdb.set_trace()
        #print(ovlp_cps.shape)
        t = ovlp_spl.t
        k = ovlp_spl.k
        ovlp_spl = interpolate.BSpline(t, ovlp_cps ,k)
        # merge into single cps
        l_merged_spl = merge_splines(l_fixed, ovlp_spl)
        r_merged_spl = merge_splines(ovlp_spl, r_fixed)

        pdb.set_trace()
        
        n_merged = len(merged_spl.c)
        print(f"{n_merged=}")
        ...
        return to_torch(merged_spl.c, **tensor_args), merged_traj 
...
def simple_merge(
    ...
    # blend trajectory 
    l_ovlp = to_numpy(left_traj.clone())
    r_ovlp = to_numpy(right_traj.clone())
    ovlp_traj = blend_2_np_trajs_23d(l_ovlp[-overlap:], r_ovlp[:overlap], blend_type="linear")

    # smoothing 
    window = 5

    #ovlp_traj = convolve(ovlp_traj, np.ones((window,2))/window, mode= 'same')
    #ovlp_traj = subsample_waypoints(out, overlap)
    #print(out.shape)
    #from scipy.ndimage import gaussian_filter1d
    #ovlp_traj = gaussian_filter1d(ovlp_traj, sigma = 2)
    #blend_traj_ref = torch.cat([left_traj, right_traj], dim=0)
    #blend_traj = to_numpy(blend_traj)
    ...
    fig_cps.savefig(os.path.join("figures/robot_trajectories_blend.png"), bbox_inches="tight")
...
def main() : 
    ...
    #     phase_time_class="PhaseTimeLinear",
    # )
    l_pts = to_torch(q_trajs_pos_all[0], **tensor_args)
    r_pts = to_torch(q_trajs_pos_all[1], **tensor_args)
    print(l_pts.shape, r_pts.shape)
    merged_q_cps, merged_traj = simple_merge_overlap(l_cps, r_cps, l_traj, r_traj, l_pts, r_pts, merged_spl = None, tensor_args=tensor_args)
    # merged_q_cps = simple_merge(l_pts,r_pts,merged_traj,tensor_args=tensor_args)
    # plot merged cps
    #plt.close(fig)
    ...
    #g_target_l = to_numpy(torch.cat(g_target_l, dim=0))
    #print(g_target_l)
