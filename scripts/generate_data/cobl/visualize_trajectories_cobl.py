...
from torch_robotics.tasks import DynPlanningTask, PlanningTask
...
@click.argument(
    "data_dir",
    type=click.Path(exists=True),
    default=Path(DATASET_BASE_DIR) / "EnvCoblEmpty2D-RobotPointMass2DBig",
)
def visualize(data_dir):
    ...
    n_trajs_display = 1  # plot this number of trajectories per task
    ...
    planning_task = PlanningTask(
        ...
        use_field_collision_self=False,
        use_field_collision_objects=False, # must add
        tensor_args=tensor_args,
    )
    ...
    task_ids_selected = np.array([56006, 56007, 56008, 56009])
    print("task_ids_selected",task_ids_selected)
    q_trajs_pos_all = []
    for task_id_selected in task_ids_selected:
        ...
        for i in np.random.choice(idxs_with_task_id, min(n_trajs_display, len(idxs_with_task_id))):
            #print(i)
            #q_trajs_pos_all.append(dataset_h5["sol_path"][i])
    ...
    # n_hrz = len(task_ids_selected)
    # for i in range(n_hrz):
    #     #axs.scatter(q_cps_all[i,:,0], q_cps_all[i,:,1], c =colors[i] , marker="o", s=10**2, zorder=100)
    #     axs.plot(q_trajs_pos_all[i][:, 0], q_trajs_pos_all[i][:, 1], linestyle="solid", linewidth=3, marker="x")
    # axs.set_ylim(env.limits[0][1], env.limits[1][1])
    # axs.set_xlim(env.limits[0][0], env.limits[1][0])
