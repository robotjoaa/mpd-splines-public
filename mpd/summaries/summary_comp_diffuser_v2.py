...
class SummaryCompDiffuserV2(SummaryBase):
    ...
    def summary_fn(
        ...
        **kwargs,
        ...
        do_cond_l = ['both_ovlp', 'both_stgl', 'st_endovlp', 'stovlp_gl']
        for _do in do_cond_l : 
            self.get_statistics(dataset, planning_task, model, control_points_idxs, train_step, tensor_args,
                                 n_samples = 10, do_cond = _do, prefix=prefix)
        ...
        control_points_normalized = apply_hard_conditioning(samples, stgl_cond)
    ...
    def get_statistics(self, dataset, planning_task, model, control_points_idxs, train_step, tensor_args, n_samples = 10, do_cond = False, prefix="") : 
        horizon = dataset.n_learnable_control_points
        do_cond_str = do_cond if do_cond else "no_cond"
        prefix = f"{prefix}_{do_cond_str} " if prefix != "" else f"{do_cond_str} " 
        ...
        for idx in control_points_idxs:
            ...
            #     samples = model.sample_unCond(batch_size=len(stgl_cond[0]))
            else:
            ...
            if do_cond == 'both_stgl' : 
                control_points_normalized_tmp = apply_hard_conditioning(samples, stgl_cond)
            else : 
                control_points_normalized_tmp = samples
