...
from mpd.models.diffusion_models.sample_functions import ddim_create_time_pairs
...
class Stgl_Sml_GauDiffusion_InvDyn_V1(nn.Module):
    ...
    def create_train_tj_cond_prev_only(self, x_clean: torch.Tensor,
                             x_noisy: torch.Tensor,
                             t_1d_st: torch.Tensor,
                             cond_st_gl: dict,
                             is_rand: bool):
        """
        Training condition that only reveals the previous overlap (no future/end overlap).
        This matches the autoregressive 'ar_prev_only' inference where each segment
        conditions on start (or previous) but never on future segments.
        """
        batch_size = x_clean.shape[0]
        device = x_clean.device
        ...
        ## No future overlap conditioning: mark as dropped and set placeholders.
        tj_cond = {
            'st_ovlp_is_drop': ~st_cd_use_ovlp,
            'end_ovlp_is_drop': torch.ones(size=(batch_size,), dtype=torch.bool, device=device),
            'st_ovlp_traj': st_traj,
            'end_ovlp_traj': None,
            'st_ovlp_t': t_1d_st,
            'end_ovlp_t': t_1d_end,
            'is_st_inpat': st_cd_use_inpat,
            'is_end_inpat': torch.zeros_like(st_cd_use_inpat, dtype=torch.bool),
        }
    ...
    def loss(self, x_clean, cond_st_gl):
        ...
        if self.diff_config.get('train_prev_only', False):
            ...
            x_noisy, tj_cond = self.create_train_tj_cond(
                x_clean, x_noisy, t_1d[:, 0], t_1d[:,0].clone(), cond_st_gl, is_rand=True)
    ...
    def ddim_p_sample_loop(
        ...
        if use_time_pairs:
            time_pairs = ddim_create_time_pairs(self.num_train_timesteps, self.ddim_num_inference_steps)
        else:
            time_pairs = [(i, i - self.num_train_timesteps // self.ddim_num_inference_steps) for i in self.ddim_set_timesteps(self.ddim_num_inference_steps)]

        from tqdm import tqdm
        for k_step, (_time, _time_next) in enumerate(tqdm(time_pairs)):
            if _time == _time_next:
                continue
            if _time_next < 0:
                _time = 1
            ...
            timesteps = torch.full((batch_size, self.horizon), _time, device=device, dtype=torch.long)
            prev_timesteps = torch.full((batch_size, self.horizon), _time_next, device=device, dtype=torch.long)
            ...
            ## ----------------------
            ### eta=0.0
            use_guide = guide is not None and (self.ddim_num_inference_steps - k_step) <= t_start_guide
            x = self.ddim_p_sample(
                ...
                guide=guide if use_guide else None,
    ...
    def ddim_p_sample(
        ...
        if prev_timesteps is not None:
            prev_timestep = prev_timesteps
        else:
        ...
        if guide is not None:
            ...
            for _ in range(n_guide_steps):
                grad_guide = guide(x_opt, tj_cond=tj_cond, g_cond=g_cond)
                if clip_grad:
                    grad_guide = torch.clamp(grad_guide, -max_grad_value, max_grad_value)
                grad_prior_weighted = grad_prior * ddim_scale_grad_prior + grad_guide
                x_opt.grad = -grad_prior_weighted
                opt.step()
                opt.zero_grad()
                x_opt.grad = None
            grad_prior = grad_prior * ddim_scale_grad_prior + grad_guide.detach()
            x = x_opt.detach()

        pred_sample_direction = (1 - alpha_prod_t_prev - std_dev_t**2) ** (0.5) * grad_prior
        ...
        for i_t in tqdm(time_idx):
            ...
            for i_tj in range(n_comp):
                ...
                if i_tj == 0:
                    ...
                    if self.use_ddim:
                        x_p_i = self.ddim_p_sample(x_p_i, tj_cond_p_i, timesteps, eta=self.ddim_eta, use_clipped_model_output=True)
                    else:
    ...
    def comp_pred_p_loop_n_same_t_parallel(self, ##  
        ...
        for i_t in tqdm(time_idx):
            ...
            if self.use_ddim:
                x_p_list_cur_t = self.ddim_p_sample(x_p_list_cur_t, tj_cond_mg, t_steps_mg, eta=self.ddim_eta, use_clipped_model_output=True)
            else:
    ...
    @torch.no_grad()
    def comp_pred_p_loop_n_ar_prev_only(self,
                        shape, stgl_cond, n_comp, do_mcmc=False, verbose=True, return_diffusion=False):
        """
        Autoregressive denoising that conditions only on the previous trajectory.
        No information from future segments is used during sampling.
        """
        raise NotImplementedError
        assert n_comp >= 1 and not do_mcmc, 'mcmc not supported.'
        device = self.betas.device
        ...
        for i_t in tqdm(time_idx):
            ...
            ## forward autoregressive: 0 -> n_comp-1
            for i_tj in range(n_comp):
                x_p_i = x_p_list[i_tj]

                if i_tj == 0:
                    stgl_c = {0: stgl_cond[0]}
                    if n_comp == 1:
                        stgl_c[hzn - 1] = stgl_cond[hzn - 1]
                    ## first traj: only start waypoint
                    x_p_i, tj_cond_p_i = self.create_eval_tj_cond(
                        x_et=x_p_i,
                        st_traj=None,
                        end_traj=None,
                    ...
                    ## others: condition on previous overlap only
                    x_p_i_minus_1 = x_p_list[i_tj - 1]
                    _, end_traj_prev = self.extract_ovlp_from_full(x_p_i_minus_1)

                    stgl_cond_dict = {}
                    if i_tj == n_comp - 1:
                        stgl_cond_dict = {hzn - 1: stgl_cond[hzn - 1]}

                    x_p_i, tj_cond_p_i = self.create_eval_tj_cond(
                        x_et=x_p_i,
                        st_traj=end_traj_prev,
                        end_traj=None,
                        t_1d_st=timesteps[:, 0],
                        t_1d_end=timesteps[:, 0],
                        t_type='0',
                        is_noisy=True,
                        stgl_cond=stgl_cond_dict,
                    )
